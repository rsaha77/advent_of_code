...
int R, C;
vector <string> grid;
map <pii, vector <int> > nums;

void create_nums () {
  rep(r, R) {
    string num = "";
    set <pii> neighbour_star_locs;
    rep (c, C + 1) {
      char ch = grid [r][c];
      if (c < C and isdigit (ch)) {
        num += ch;
        rep (j, 8) {
          int nr = r + dir [j][0];
          int nc = c + dir [j][1];
          if (ins (nr, nc, R, C)) {
            int nch = grid [nr][nc];
            if (nch == '*') {
              neighbour_star_locs.insert ({nr, nc});
            }
          }
        }
      } else if ((c == C or !isdigit (ch)) and num != "") {
        for (pii star_loc: neighbour_star_locs) {
          nums [star_loc].push_back (stoll (num));
        }
        num = "";
        neighbour_star_locs.clear();
      }
    }
  }
}
