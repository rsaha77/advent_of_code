...
def dfs (grid, r, c, pr, pc, R, C, curr_step, total_steps, ans):
  ...
  for dr,dc in direction:
    ...
    if ins(nr, nc, R, C) and grid[nr][nc] != '#':
      dfs(grid, nr, nc, r, c, R, C, curr_step, total_steps, ans)
...
""" Below needs DP??!! else will be extremely slow
"""
# ans=set()
# dfs(grid, sr, sc, -1, -1, R, C, 0, total_steps, ans)
# print(len(ans))
...
while steps < total_steps:
  ...
  print(f"steps: {steps}, ans = {sum (1 for row in step_grid for ch in row if ch == 'O')}")
